...
(function() {
    ...
    function bmRenderBarChart(data, title) {
        ...
        requestAnimationFrame(function() {
            container.querySelectorAll('.bm-bar-fill').forEach(function(bar) { bar.style.width = bar.getAttribute('data-width'); });
        });
    ...
    function bmDrillToReportInline(type, period, subject, subjectId) {
        ...
        drillSection.innerHTML = '<div class="flex items-center justify-between mb-4"><h3 class="text-lg font-bold text-gray-900">Carregando...</h3><button class="bm-close-drill text-sm text-blue-600 hover:underline">← Voltar para Visão Geral</button></div>';
        ...
        .then(function(response) {
            if (response.success && response.data) {
                ...
                var html = '<div class="flex items-center justify-between mb-4"><h3 class="text-lg font-bold text-gray-900">' + (data.title || 'Relatório') + '</h3><button class="bm-close-drill text-sm text-blue-600 hover:underline">← Voltar para Visão Geral</button></div>';
    ...
    function bmCreateKPICard(label, value, variance, period, drillType, drillPeriod, drillSubject, drillSubjectId) {
        ...
        card.innerHTML = '<div class="flex items-center justify-between"><div>' +
            ...
            periodHtml;
        card.addEventListener('click', function() { if (drillType) bmDrillToReport(drillType, drillPeriod, drillSubject || 'all', drillSubjectId || ''); });
        return card;
    }
    ...
    function bmCreateHighlightCard(type, data, period) {
        ...
        if (type === 'student') {
            ...
            content = '<div class="text-3xl mb-2">👤</div><p class="text-lg font-bold text-gray-900">' + (data ? (data.name || '—') : '—') + '</p><p class="text-sm text-gray-500">' + (data ? (data.books_read || 0) : 0) + ' livros</p>';
        } else if (type === 'book') {
            drillType = 'top_books'; drillSubject = 'all'; drillSubjectId = '';
            content = '<div class="text-3xl mb-2">📖</div><p class="text-lg font-bold text-gray-900">' + (data ? (data.title || '—') : '—') + '</p><p class="text-sm text-gray-500">' + (data ? (data.author || '') : '') + '</p><p class="text-sm text-gray-500">' + (data ? (data.loans || 0) : 0) + ' empréstimos</p>';
        } else if (type === 'revelation') {
            drillType = 'student_performance'; drillSubject = 'student'; drillSubjectId = data ? (data.id || '') : '';
            content = '<div class="text-3xl mb-2">🌟</div><p class="text-lg font-bold text-gray-900">' + (data ? (data.name || '—') : '—') + '</p><p class="text-sm text-gray-500">+' + (data ? (data.increase || 0) : 0) + ' livros vs período anterior</p>';
        }
    ...
    function bmCreateUtilityCard(title, data, period, drillType, icon) {
        ...
        if (Array.isArray(data)) {
            if (data.length === 0) {
                contentHtml = '<p class="text-xs text-gray-400 py-2">Nenhum dado no período</p>';
            } else {
